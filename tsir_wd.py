...
def tSIR_WD_beta_CRN(N, v, i0, beta, T, pop_seed, dyn_seed):
    """
    Weak-derivatives for the tSIR model, with respect to beta.
    
    Returns a list where ith entry contains the scaling factor, 
    the plus trajectory, and the minus trajectory generated by perturbing the 
    next infectives R.V generated at time i.
    The simulation paths of the two trajectories first differ at i+1, 
    due to the use of CRN coupling. But each pair of trajectories in the list
    is sampled independently.
    
    Returns:
        list[(float, np.ndarray(T+1, 3), np.ndarray(T+1, 3))] of length T.
    """
    pop_rng = np.random.default_rng(pop_seed)
    dyn_rng = np.random.default_rng(dyn_seed)
...
def fd_matrix_powers_I(N, v, i0, beta, T, eps, param = 'beta'):
    """
    Use the matrix-powers approach to calculate the expected number of 
    infected individuals at a certain time, then use a centered finite 
    difference to calculate the derivative with respect to one of the parameters
    of interest.
    """
    if param == 'beta':
...
def one_sided_fd_crn(N, v, i0, beta, T, eps, pop_seed, dyn_seed, sample_perf, param = 'beta'):
    """
    A one sided finite-difference estimator (with CRN) for derivatives of 
    a sample performance with respect to v or beta.
